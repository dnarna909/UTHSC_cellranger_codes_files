...
## add files -------------------
```{r}
Disk1 <- c("/media/jianie/Extreme SSD1/") 
# Disk <- c("D:/")
...
Disk5 <- c("/media/jianie/Extreme SSD1/") 
# Disk5 <- c("D:/")
...
STARR.samples.basic1 <- rio::import(paste0(Disk1, file.folder1, "/", basic.file1)) %>% 
  ...
  mutate(Diabetes = ifelse(dbc4_diabetes == 2, "Yes", "No")) %>% 
  rename(Blood.Pressure.Systolic = systolic,
         ...
         LDL = ldl,                                  
         VLDL = vldl            )%>%
  mutate(Race = ifelse(race___1 == 2, "Yes", "No"))
...
table(STARR.samples.basic1$dbc4_diabetes)
# dbc4_diabetes
...
Disk2 <- c("/media/jianie/DATA/") 
# Disk2 <- c("C:/Users/niej/Documents/")
file.folder2 <- c("UTHSC_cellranger_codes_files/RedCap_SubjectsInfo")  # download from online by Allison
...
sglt2.samples <- sglt2.samples.basic1 %>%
  select(-one_of(setdiff(intersect(colnames(sglt2.samples.basic1), colnames(sglt2.samples.basic2)) , merge.cols )) 
  )  %>% 
...
# ss=sample_ids[1]
sglt2[[stringr::str_split(ss, "_")[[1]][1]]][[ifelse(is.na(stringr::str_split(ss, "_")[[1]][2]), "Others", stringr::str_split(ss, "_")[[1]][2])]] <- sglt2.samples %>% filter(sample_id == ss) %>% mutate_all(~na_if(., '')) %>% select_if(function(x) !(all(is.na(x)) | all(x==""))) %>% select(!one_of(cols))
# print(ss)
}
rm(cols, ss)
...
colnames(sglt2.combine4 %>% filter(Pre_Post == "Pre") %>% select_if(is.character))
cols = c("patient_id" , "Gender" ,"Race", "dbc4_diabetes", 
  "Group.Assignment." , "Diabetes" , "Age_Group"  , "Status" , "Group")   
basal <- sglt2.combine4 %>% filter(Pre_Post == "Pre") %>% select(one_of(cols))

sglt2.combine5 <- sglt2.combine4 %>% select(!one_of(setdiff(cols, "patient_id"))) %>%
  left_join(basal, by = "patient_id")

# adding more colums
table(sglt2.combine4$dbc4_diabetes)
sglt2.prediabetes <- sglt2.combine4 %>% filter(dbc4_diabetes == "No") %>% pull(patient_id)
...
# sglt2.sn.V1 <- sglt2.samples %>% dplyr::filter(`Event.Name` == "Visit 1") %>% arrange(id) %>% select(all_of(sglt2.V1.cols)) %>% 
...
#                    "Age", "Age_Group"
# )
# sglt2.sn.V2 <- sglt2.samples %>% dplyr::filter(`Event.Name` == "Visit 2") %>% arrange(id) %>% select(all_of(sglt2.V2.cols)) %>% 
...
# )
# sglt2.sn.V5 <- sglt2.samples %>% dplyr::filter(`Event.Name` == "Visit 5") %>% arrange(id) %>% select(all_of(sglt2.V5.cols)) # ,  SingleNuclei == "Yes"
...
#                     "Height_inches" , "Weight_pounds",  "BMI", "Status",
#                     "Glucose" , "A1c_Value")
# sglt2.sn.V11 <- sglt2.samples %>% dplyr::filter(`Event.Name` == "Visit 11") %>% arrange(id) %>% select(all_of(sglt2.V11.cols)) # ,  SingleNuclei == "Yes"
# sglt2.sn.V11 <- sglt2.sn.V11 %>% 
#   left_join(sglt2.sn.V1 %>% select(!all_of(c("Glucose" , "A1c_Value"))) , by = c("Record.ID..766.", "patient_id")) %>%
#   left_join(sglt2.sn.V2, by = c("Record.ID..766.", "patient_id")) %>%
...
#   select(-one_of(setdiff(intersect(colnames(sglt2.samples), colnames(sglt2.sn.V11)) , "id" ))) %>% 
