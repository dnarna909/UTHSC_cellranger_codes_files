...
sglt2.samples.basic1 <- rio::import(paste0(Disk1, file.folder1, "/", basic.file1)) %>% 
  ...
  left_join(
    rio::import(paste0(Disk1, file.folder1, "/", basic.file1.1)) %>% 
      select(one_of(c("Record ID: 766-", "Event Name", "Ethnicity"))) %>%
      as.data.frame() %>% 
      rename_with(make.names) %>% 
      as.data.frame() , 
...
for (ss in sample_ids) {
  # ss=sample_ids[1]
  sglt2[[stringr::str_split(ss, "_")[[1]][1]]][[ifelse(is.na(stringr::str_split(ss, "_")[[1]][2]), "Others", stringr::str_split(ss, "_")[[1]][2])]] <- sglt2.samples %>% dplyr::filter(sample_id == ss) %>% mutate_all(~na_if(., '')) %>% select_if(function(x) !(all(is.na(x)) | all(x==""))) %>% select(!one_of(cols))
  # print(ss)
}
rm(cols, ss)
...
colnames(sglt2.combine4 %>% dplyr::filter(Pre_Post == "Pre") %>% select_if(is.character))
cols = c("patient_id" , "Gender" ,"Race", "dbc4_diabetes", "Ethnicity",
         "Group.Assignment." , "Age_Group"  , "Status" ,"Prediabetes1", "Prediabetes2" # , "Diabetes"  , "Group"
)   
basal <- sglt2.combine4 %>% dplyr::filter(Pre_Post == "Pre") %>% select(one_of(cols))
