...
xlsx::write.xlsx(sequencing.info, file = "/media/jianie/DATA/UTHSC_cellranger_codes_files/2022-08-18 Prepare for UTSA/Sequencing Files_combine.xlsx", row.names=FALSE)
...
sglt2.sn.V11 <- sglt2.sn.V11 %>% left_join(sglt2.sn.V1 %>% select(!all_of(c("Glucose" , "A1c Value"))) , by = c("Record ID: 766-", "patient_id")) %>%
  left_join(sglt2.sn.V5 %>% select(all_of(c("patient_id" , "Group Assignment:"))), by = c("patient_id"))
...
```{r}
SGLT2.agg <- readRDS(file = "/media/jianie/DATA/UTHSC_cellranger_codes_files/AggrFiles/SGLT2.sn.aggr.info.rds") %>% rename(Status.counts = Status)
intersect(colnames(sglt2.sn.all), colnames(SGLT2.agg))
...
xlsx::write.xlsx(SGLT2.sn, file = "/media/jianie/DATA/UTHSC_cellranger_codes_files/RedCap_SubjectsInfo/SGLT2.sn.aggr.combine.info.xlsx", sheetName = "STARR", row.names=FALSE)
