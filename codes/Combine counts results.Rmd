...
```{r}
aggr <- rio::import("/media/jianie/DATA/Single-Cell-Sample-Sheet/cellranger/aggregation.xlsx")
aggr2 <- aggr[-which(aggr$Status == "Delete"), ]
df.all <- data.frame()
for (pp in aggr2$Metrics) {
  ...
  dx <- add_column(dx, List = aggr2[which(aggr2$Metrics == pp), "List"], .before = 1)
...
df.all2 <- merge(df.all, aggr2, by = c("List", "sample_id_ori"))
```
...
write.csv(STARR[c("sample_id", "molecule_h5", "sample_id_ori", "List")], file = "STARR.agg.csv")
```
SGLT2 project
```{r}
SGLT2 <- df.all2[which(df.all2$Project == "SGLT2" & df.all2$Agg == "Yes"), ]
write.csv(SGLT2[c("sample_id", "molecule_h5")], file = "SGLT2.agg.csv")
```
...
STARR2 <- df.all2[which((df.all2$Project == "STARR" | df.all2$sample_id %in% c("76615-V5", "76632-V5", "76638-V5", "76643-V5",
                         ...
                         df.all2$Agg == "Yes") , ]
write.csv(STARR2[c("sample_id", "molecule_h5")], file = "STARR2.agg.csv")
```
