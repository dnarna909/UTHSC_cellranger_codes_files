...
df.all4 <- merge(sum, df.all3, by = c("Sequencing_Date", "Lane"))
df.all4 <- add_column(df.all4, Percent.PerLane = ifelse(df.all4$Status == "Original", 
                                                        round(df.all4$`Number of Reads`/df.all4$Sum.Sequencing *100, digits = 1), NA), 
                      .after = 3)
df.all4 <- add_column(df.all4, Diff.PerLane = ifelse(df.all4$Status == "Original", 
                                                        round(df.all4$Percent.PerLane - (df.all4$`Target %` *100), digits = 1), NA), 
                      .after = 4)
df.all4 <- df.all4[order(df.all4$SQ_List),]
```


```{r chunk 2}
saveRDS(df.all4, file = "/media/jianie/DATA/UTHSC_cellranger_codes_files/ExperimentSummaryFiles/Form 4_Counts_Sequencing_Combine.Rds")
write.csv(df.all4, file = "/media/jianie/DATA/UTHSC_cellranger_codes_files/ExperimentSummaryFiles/Form 4_Counts_Sequencing_Combine.csv", row.names = FALSE)
```
